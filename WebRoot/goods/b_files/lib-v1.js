if (document.getElementById("nav-minitiao")) {
	document.getElementById("nav-minitiao").innerHTML = '<a target="_blank" href="">\u8ff7\u4f60\u6311</a>'
}
	...
	jQuery.query = new function() {
			...
			if (m.queryObject) {
				...
				jQuery.each(arguments, function() {
					...
					jQuery.each(o.split(/[&;]/), function() {
						...
						var q = decodeURIComponent(encodeURIComponent(this
								.split("=")[1]));
						if (!p) {
							return
						}
		...
		k.prototype = {
			...
			load : function(m) {
				var o = m.replace(/^.*?[#](.+?)(?:\?.+)?$/, "$1");
				var n = m.replace(/^.*?[?](.+?)(?:#.+)?$/, "$1");
				return new k(m.length == n.length ? "" : n,
						m.length == o.length ? "" : o)
			},
			empty : function() {
...
Date.prototype.toString = Date.prototype.toString.overwrite(function(b) {
	...
	if (typeof (b) == "string") {
		...
		a = a.replace(/MM/, this.getMonth() >= 9 ? this.getMonth() + 1 : "0"
				+ (this.getMonth() + 1));
		a = a.replace(/M/, this.getMonth());
		a = a.replace(/dd|DD/, this.getDate() > 9 ? this.getDate() : "0"
				+ this.getDate());
		a = a.replace(/d|D/, this.getDate());
		a = a.replace(/hh|HH/, this.getHours() > 9 ? this.getHours() : "0"
				+ this.getHours());
		a = a.replace(/h|H/, this.getHours());
		a = a.replace(/mm/, this.getMinutes() > 9 ? this.getMinutes() : "0"
				+ this.getMinutes());
		a = a.replace(/m/, this.getMinutes());
		a = a.replace(/ss|SS/, this.getSeconds() > 9 ? this.getSeconds() : "0"
				+ this.getSeconds());
		...
		while (endStmtPrev + 1 < body.length) {
			...
			while (begStmt >= 0) {
				...
				if (blockrx) {
					...
					if (blockMarkerEnd >= 0) {
						...
						if (blockEnd >= 0) {
							...
							if (blockType == "cdata") {
								...
								if (blockType == "minify") {
									emitText(scrubWhiteSpace(blockText),
											funcText)
								} else {
									if (blockType == "eval") {
										if (blockText != null
					...
					if (body.charAt(begStmt - 1) != "$"
							&& body.charAt(begStmt - 1) != "\\") {
						var offset = (body.charAt(begStmt + 1) == "/" ? 2 : 1);
						if (body.substring(begStmt + offset,
								begStmt + 10 + offset).search(
...
(function(a) {
	a.fn.Jtab = function(d, h) {
		...
		var f = a(this).find("*[" + b.hookKey + "=" + b.hookItemVal + "]"), e = a(
				...
				.toLowerCase()
				.match(
						/http:\/\/|\d|\.aspx|\.ascx|\.asp|\.php|\.html\.htm|.shtml|.js/g);
...
(function(a) {
	a.fn.Jlazyload = function(j, n) {
		...
		var f = a.extend( {
			...
			var v = document.documentElement, r = document.body, u = window.pageXOffset ? window.pageXOffset
					: (v.scrollLeft || r.scrollLeft), t = window.pageYOffset ? window.pageYOffset
					: (v.scrollTop || r.scrollTop), s = v.clientWidth, q = v.clientHeight;
			return {
				left : u,
				top : t,
				width : s,
			...
			a.each(k,
					function() {
						...
						var s = (!f.offsetParent) ? m : l(a(f.offsetParent)
								.get(0)), r = l(this), q = d(s, r);
						switch (f.type) {
						case "image":
							b(q, t, a(this));
							break;
...
(function(a) {
	a.Jtimer = function(d, h) {
		var c = a.extend( {
			...
			var j = g(p.start), l = g(p.server), n = g(p.end), v, u, o, s = (j - l) / 1000, m = (n - l) / 1000, w = "#"
					+ c.mainPlaceholder + t, k = "#" + c.subPlaceholder + p.qid, r = "#"
					+ c.resetPlaceholder + p.qid;
			if (s <= 0) {
				var q = c.template.process(p);
				a(w).html(q)
			}
			c.timer[p.qid] = setInterval(
					function() {
						if (s > 0) {
							...
							if (m > 0) {
								...
								if (c.iconPlaceholder) {
									iconElement = "#" + c.iconPlaceholder
											+ p.qid;
									a(iconElement).attr("class",
											c.finishedClass).html(
											"\u62a2\u5b8c")
								}
								if (c.reset) {
									a(k)
											.append(
													'<a href="javascript:void(0)" id="' + r
															.substring(1) + '">\u5237\u65b0</a>');
	...
	a.fn.Jslider = function(h, n) {
		...
		var g = a(this), e = null, k = null, j = null, c = null, m = null, o = function() {
			...
			if (d.reInit) {
				...
				var t = r.itemsWrap.replace("{innerHTML}", r.itemsContent)
						+ r.controlsWrap.replace("{innerHTML}",
								r.controlsContent), q = t.process(p);
				g.html(q)
			}
...
jQuery.fn.pagination = function(a, b) {
	...
	return this.each(function() {
		...
		function h() {
			...
			var j = g > l ? Math.min(g + l, m) : Math.min(
		...
		function c() {
			...
			var n = function(q, r) {
				...
				if (q == g) {
					var s = $("<a href='javascript:void(0)' class='current'>"
							+ (r.text) + "</a>")
				} else {
					var s = $("<a>" + (r.text) + "</a>").bind("click", p(q))
							.attr("href", b.link_to.replace(/__id__/, q))
				}
				if (r.classes) {
...
(function(a) {
	a.extend(a.browser, {
		...
		isMinW : function(b) {
			return Math.min(a.browser.client().bodyWidth,
					a.browser.client().width) <= b
		},
		isMinH : function(b) {
	...
	a.fn.jdPosition = function(f) {
		...
		case "center":
			...
			a(this)
					.css(
							{
								left : a.browser.scroll().left
										+ Math
												.max(
														(a.browser.client().width - c) / 2,
														0) + "px",
								top : (!a.browser.isIE6) ? (d ? a.browser
										.scroll().top
										: (a.browser.scroll().top
												+ Math
														.max(
																(a.browser
																		.client().height - g) / 2,
																0) + "px"))
										: ((a.browser.scroll().top <= a.browser
												.client().bodyHeight
												- g) ? (a.browser.scroll().top
												+ Math
														.max(
																(a.browser
																		.client().height - g) / 2,
																0) + "px")
												: (a.browser.client().height - g)
														/ 2 + "px")
							});
			break;
		case "auto":
...
(function(a) {
	a.fn.jdThickBox = function(f, k) {
		...
		var m = function() {
			...
			if (o._autoReposi) {
				a(window).unbind("resize.jdThickBox").unbind(
						"scroll.jdThickBox")
			}
		};
		if (o.close) {
		...
		var n = function(p) {
			if (a(".thickframe").length == 0 || a(".thickdiv").length == 0) {
				...
				a("<div class='thickdiv' id='" + d(o._div) + "'></div>")
						.appendTo(a(document.body))
			} else {
				a(".thickframe").add(".thickdiv").show()
			}
			a("<div class='thickbox' id='" + d(o._box) + "'></div>").appendTo(
					a(document.body));
			if (o._titleOn) {
				h(p)
			}
			a(
					"<div class='thickcon' id='" + d(o._con)
			...
			l(p);
			if (o._autoReposi) {
				a(window).bind("resize.jdThickBox", g).bind(
						"scroll.jdThickBox", g)
			}
			if (o._fastClose) {
		...
		var b = function() {
			var p = o._countdown;
			a(
					"<div class='thickcountdown' style='width:"
							+ o.width
							+ "'><span id='jd-countdown'>"
							+ p
							+ "</span>\u79d2\u540e\u81ea\u52a8\u5173\u95ed</div>")
					.appendTo(a(".thickbox"));
			c = setInterval(function() {
		...
		var h = function(p) {
			...
			a(
					"<div class='thicktitle' id='" + d(o._title)
							+ "' style='width:" + o.width + "'><span>"
							+ o.title + "</span></div>").appendTo(
					a(".thickbox"))
		};
		var j = function() {
			if (o._close != null) {
				a(
						"<a href='#' class='thickclose' id='" + d(o._close)
								+ "'>" + o._close_val + "</a>").appendTo(
						a(".thickbox"));
		...
		var l = function(p) {
			...
			case "image":
				...
				a(".thickcon").append(
						"<img src='" + o.source + "' width='" + o.width
								+ "' height='" + o.height + "'>");
				o.source = null;
				a(".thickcon").removeClass(o._loading);
				if (k) {
		...
		var g = function() {
			var q = a(".thickcon").outerWidth(), t = (o._titleOn ? a(
					".thicktitle").outerHeight() : 0)
					+ a(".thickcon").outerHeight();
			a(".thickbox").css( {
				width : q + "px",
				height : t + "px"
			});
			...
			if (a.browser.isIE6) {
				var s = a(".thickbox").outerWidth(), u = a(".thickbox")
						.outerHeight();
				var p = a.browser.isMinW(s), r = a.browser.isMinH(u);
				a(".thickframe").add(".thickdiv").css(
						{
							width : p ? s : "100%",
							height : Math.max(a.browser.client().height,
									a.browser.client().bodyHeight)
									+ "px"
						})
			}
		};
...
(function(a) {
	a.fn.jdMarquee = function(h, b) {
		...
		var o = function() {
			var s = "<div style='position:relative;overflow:hidden;z-index:1;width:"
					+ j.width
					+ "px;height:"
					+ j.height
					+ "px;"
		...
		var q = function() {
			var v = (j.deriction == "up" || j.deriction == "down") ? parseInt(u
					.get(0).style.marginTop)
					: parseInt(u.get(0).style.marginLeft);
			var w = (j.deriction == "up" || j.deriction == "down") ? parseInt(r
					.get(0).style.marginTop)
					: parseInt(r.get(0).style.marginLeft);
			var x = Math.max(Math.abs(v - j.pos.object), Math.abs(w
					- j.pos.clone));
	...
	if (a.loginService != "" && a.loginMethod != "") {
		$.getJSON(a.loginService, {
			...
			if (b != null) {
				...
				if (!b.Identity.IsAuthenticated && a.automatic
						&& a.loginUrl != "") {
					if (a.modal) {
						jdModelCallCenter.login()
					} else {
						location.href = a.loginUrl + "?ReturnUrl="
								+ escape(a.returnUrl)
					}
				}
			}
...
(function(a) {
	a.jdCalcul = function(g) {
		var b = null;
		var g = g.join(",");
		var e = "http://qiang.360buy.com/HomePageLimitBuy.ashx?callback=?&ids="
				+ g;
			...
			var h = function() {
				...
				a
						.each(
								r.pros,
								function(w) {
									var A = r.pros[w].id, z = r.pros[w].tp, x = (r.pros[w].zt == 1) ? "<div class='pi9'></div>"
											: "<div class='pi10'></div>", y = unescape(r.pros[w].mc), u = r.pros[w].qg, v = "("
											+ r.pros[w].zk + "\u6298)";
									t += s.replace(/\{0\}/g, A).replace("{1}",
											z).replace("{2}", x).replace("{3}",
											y).replace("{4}", u).replace("{5}",
											v).replace(/\{6\}/g, d)
								});
				t += "</ul>";
				r.contentid.html(t)
...
function mlazyload(d) {
	...
	c = $.extend(c, d || {});
	var b = c.defHeight, f = (typeof c.defObj == "object") ? c.defObj
			: $(c.defObj);
	if (f.length < 1) {
		return
	}
	var a = function() {
		var g = document, h = (navigator.userAgent.toLowerCase().match(/iPad/i) == "ipad") ? window.pageYOffset
				: Math.max(g.documentElement.scrollTop, g.body.scrollTop);
...
var jdRecent = {
	...
	getData : function(a) {
		if (a == 0) {
			this.element
					.html("<li><div class='norecode'>\u6682\u65e0\u8bb0\u5f55!</div></li>");
			return
		}
		var b = a.split(".");
		for (i in b) {
	...
	setData : function(a) {
		this.element.append("<li><div class='p-img'><a href='" + a.url
				+ "'><img src='" + a.img
				+ "' /></a></div><div class='p-name'><a href='" + a.url + "'>"
				+ decodeURIComponent(a.name) + "</a></div></li>")
	}
};
...
var jdModelCallCenter = {
	...
	login : function() {
		this.tbClose();
		var c = this;
		var b = navigator.userAgent.toLowerCase(), a = (b.match(/ucweb/i) == "ucweb" || b
				.match(/rv:1.2.3.4/i) == "rv:1.2.3.4");
		if (a) {
	...
	autoLocation : function(a) {
		...
		$
				.login( {
					modal : true,
					complete : function(c) {
						if (c != null && c.IsAuthenticated != null
								&& c.IsAuthenticated) {
							window.location = a
						}
					}
											...
											if (a > 0) {
												$
														.getJSON(
																...
																function(e) {
																	if (e.code != null) {
																		var g = (e.code == "success") ? 510
																				: 388, d = (e.code == "success") ? 500
																				: 80, f = (e.code == "success") ? false
																				: 6;
																		$
																				.jdThickBox( {
																					type : "iframe",
																					source : "http://t.360buy.com/subscriber/jumpSubPageNew.action?code="
																							+ e.code
																							+ "&pid="
																							+ a
																							+ "&t="
...
var category = {
	...
	getBrandService : function(a) {
		...
		$(".cat-right-con").each(
				function(d) {
					$(this).find(".categorys-promotions").attr("clstag",
							"homepage|keycount|home2012|0" + (601 + d) + "c");
					$(this).find(".categorys-brands").attr("clstag",
							"homepage|keycount|home2012|0" + (601 + d) + "d")
				})
	},
	FN_Init : function() {
		if (!this.OBJ.length) {
...
var UC = {
	...
	FN_InitVList : function(a) {
		...
		$.each(b, function(c) {
			...
			$.getJSONP(
					"http://www.360buy.com/lishiset.aspx?callback=UC.FN_ShowVList&id="
							+ b[c], UC.FN_ShowVList)
		})
	},
	...
	FN_InitOList : function() {
		...
		$.ajax( {
			...
			success : function(a) {
				if (a && a.error == 0) {
					$("#num-unfinishedorder")
							.html(UC.FN_setWords(a.orderCount))
				}
			}
		});
		$
...
var MCART = {
	...
	FN_BindEvents : function() {
		$("#settleup-content").find(".delete").bind("click", function() {
			var b = $(this).attr("id"), a = $(this).attr("name");
			if (!b) {
				return
			}
			$.ajax( {
	...
	FN_Refresh : function() {
		var b = $("#settleup dl");
		var d = b.find("dd").eq(0), c, a = function(e) {
...
(function() {
	...
	document.onkeyup = function(c) {
		...
		var c = c ? c : window.event, b = c.keyCode || c.which;
		switch (b) {
		case 68:
	...
	a.prototype.mouseOverItem = function(s) {
		...
		if (-1 != this.index && null != s.id && !s.id.trim().isEmpty()) {
			...
			if (2 == t.length) {
				...
				if (-1 != this.HIndex) {
					$("#d_" + this.index + "_" + this.HIndex).css(
	...
	a.prototype.onloadItems = function(ap) {
		...
		for ( var aw = 0, av = 0, O = F; aw < O; aw++) {
			...
			if ("string" == typeof (w.cid) && !w.cid.trim().isEmpty()) {
				...
				var al = "";
				if ("3" == H && null != w.expand && (w.expand instanceof Array)
						&& w.expand.length > 0) {
					M = true
				} else {
					M = false
				...
				if (M) {
					...
					loopEs: for ( var au = 0; au < s; au++) {
						var aq = w.expand[au];
						var aa = aq.esname;
						if (null == aa || "undefined" == typeof (aa)
								|| "" == aa) {
							continue loopEs
						}
						var ae = aq.esid;
						if (null == ae || "undefined" == typeof (ae)
								|| "" == ae) {
							continue loopEs
						}
						av += 1;
...
pageConfig.FN_InitSidebar = function() {
	...
	this.initCss = function() {
		...
		if (screen.width >= 1210) {
			if ($.browser.msie && $.browser.version <= 6) {
				...
				b = {
					right : (document.documentElement.clientWidth - c) / 2 - 26
							+ "px"
				}
			}
			a.css(b)
	...
	this.setTop = function() {
		this
				.addItem("<a href='#' class='gotop' title='\u4f7f\u7528\u5feb\u6377\u952eT\u4e5f\u53ef\u8fd4\u56de\u9876\u90e8\u54e6\uff01'><b></b>\u8fd4\u56de\u9876\u90e8</a>")
	}
};
pageConfig.FN_InitContrast = function(d, c, b) {
	var d = d || "_contrast", b = b || "list", c = c
			|| "http://misc.360buyimg.com/contrast/js/contrast.js", a = readCookie(d
			+ "_status");
	if (a == "show" || a == "side" || !!readCookie(d) == true) {
		...
		console.log("o");
		$(".btn-compare").unbind("click").bind("click", function() {
