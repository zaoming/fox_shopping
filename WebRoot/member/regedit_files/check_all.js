		   ...
		   $.ajax({
				  ...
				  success: function(response){
					  ...
					  if((trim(response))==1){
						 ...
						 var email = jQuery('#email').val();
						showEmailError("邮箱已被注册，请重新输入或使用该邮箱<a href='login.jsp?email="+email+"'>登录</a>");
				  ...
				  success: function(response){
					  ...
					  if(response==1){ $("#name_desc").removeClass("extra");
						...
						$("#name_desc").text("").show();}
						  ...
						  var name=jQuery('#name').val();
						  hideOtherTips("none");
						  showNameError("用户名已被注册，请重新输入或使用该用户名<a href='login.jsp?email="+name+"'>登录</a>");
					  }
					  					  }
			});
			}
