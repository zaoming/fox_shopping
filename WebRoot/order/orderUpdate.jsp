...
<link rel="shortcut icon"
		...
		<link rel="stylesheet" type="text/css"
			href="./orderdetails_files/global_yhdLib.css">
		<script type="text/javascript"
			src="./orderdetails_files/global_yhdLib.js"></script>
		<script type="text/javascript"
			src="./orderdetails_files/global_login_frame.js"></script>
...
<link href="./editinfo_files/WdatePicker.css" rel="stylesheet" type="text/css"><link rel="stylesheet" type="text/css" href="./editinfo_files/global_yhdLib.css"><script type="text/javascript" src="./editinfo_files/global_yhdLib.js"></script><script type="text/javascript" src="./editinfo_files/global_login_frame.js"></script></head>
			...
			<div id="myYihaodianLeftMenu" style="display: block; float: left;"
				...
				<div class="columnright">
					<div class="account_box">
						<span class="fr red">&nbsp;</span><em>订单编号</em>
						<strong>${sessionScope.orderVo1.orderID }</strong>
					</div>
					<p class="mb10">
						<br />
					</p>
					<div class="editbox">
							...
							<tbody>
								<tr>
									<th width="250" colspan="2">
										商品
										<input type="hidden" id="orderId" value="79972371">
									</th>
									<th>
										<strong>91商城价</strong>
									</th>
								...
								<c:forEach items="${orderDetailList1 }" var="odl">
								<tr>
									<td width="65">
										<a href="<%=path %>/goods/goodsDetail?goodsID=${odl.goodsID }"> <img width="50" height="60" src="${odl.goodsPicture }" /> </a>
									</td>
									<td style="border-left: none; text-align: left" width="185">
										<a href="<%=path %>/goods/goodsDetail?goodsID=${odl.goodsID }">${odl.goodsName }</a>
									</td>
									<td>
										${odl.goodsPrice }
										<br>
						...
						<div class="ordernum">
							<strong class="f14"><span>运费：</span>¥10</strong><br/>
							<strong class="f14"><span>商品总金额：</span>¥ ${orderVo1.orderAmount }</strong>
							<br>
							<div class="clear"></div>
						</div>
					</div>
					...
					<script>
						function change(){
							...
							var edit_add = document.getElementById("edit_add");
					...
					<div class="editbox" id="address"  style="display:block;">
						<h2>
							<a href="javascript:void(0);" onclick="change()" id="editOrderAddress">修改</a>收货地址
						</h2>
						<ul class="re_address">
							<li>
								<label>
								...
								${sessionScope.orderVo1.memberProvince } ${sessionScope.orderVo1.memberCity } ${sessionScope.orderVo1.memberAddress }
							...
							<li>
								...
								<span id="receiver">${sessionScope.orderVo1.memberTrueName }</span>
							</li>
							<li>
								<label>
									邮政编码：
								</label>
								<span id="postCode">${sessionScope.orderVo1.memberPostcode }</span>
							</li>
							<li>
								<label>
									手机：
								</label>
								<span id="cellphone">${sessionScope.orderVo1.memberTelephone }</span>
							</li>
						</ul>
					</div>
					<div id="waitInfo"></div>
					
					<form name="fm" method="post" action="<%=path %>/orderUpdate_1?orderID=${sessionScope.orderVo1.orderID }">
					
					<div class="editbox editing none" style="display: none;"  id="edit_add">
						<h2>
							收货地址
						</h2>
						<ul class="re_address">
							<input type="hidden" id="receiverID" value="12625057"
								oid="79972371" "="" provinceid="16" cityid="170" countyid="1694"
								areaid="1694">
							<li>
								<label>
									收货人：
								</label>
								<input id="receiverPerson" type="text" name="memberTrueName" value="${sessionScope.orderVo1.memberTrueName }">
							</li>
							<li>
								<label>
									所在地区：
								</label>
								${sessionScope.orderVo1.memberProvince } ${sessionScope.orderVo1.memberCity }
								
								<input type="hidden" value="${sessionScope.orderVo1.memberProvince }" name="ismemberProvince"/>
								<input type="hidden" value="${sessionScope.orderVo1.memberCity }" name="ismemberCity"/>
							...
							<li>
								...
								<input id="receiveAddr" type="text" name="memberAddress" value="${sessionScope.orderVo1.memberAddress }" size="45">
							</li>
							<li>
								<label>
									邮政编码：
								</label>
								<input id="postCode" type="text" name="memberPostcode" maxlength="6" value="${sessionScope.orderVo1.memberPostcode }">
							</li>
							<li>
								<label>
									手机：
								</label>
								<input id="receiveTel" type="text" name="memberTelephone" maxlength="11" value="${sessionScope.orderVo1.memberTelephone }">
							</li>
						
						</ul>
						<div class="btnarea">
							<img src="./myorderupdate_files/orderMUpdate.png" onclick="javascript:document.fm.submit();"/>
						</div>
						</form>
					</div>
    ...
    if(lazyLoadImageObjArry){ 
    	...
    	lazyLoadImageObjArry.push("footerbanner2LazyLoad");
			...
			<!--无页面级头部js时, 全局头部js下移-->
			<script type="text/javascript"
				src="./myorderupdate_files/global_site_top.js" charset="utf-8"></script>
			<script type="text/javascript"
				src="./myorderupdate_files/editOrder.js" charset="utf-8"></script>
			<script type="text/javascript"
				src="./myorderupdate_files/global_site_bottom.js" charset="utf-8"></script>
			<!--js end-->
			<div>
				<a style="color: #FFFFFF"> 119!$ 0%&amp; 2#@ 10!$ , </a>
